...
class CGFormer_v2(BaseModule):
    def __init__(
        ...
        # refinment_head
        if refinment_head is not None:
            self.refinment_head = builder.build_head(refinment_head)
    ...
    def forward_train(self, data_dict):
        ...
        voxel_feats_enc = self.occ_encoder_backbone(img_voxel_feats)
        ...
        if self.refinment_head is not None:
            output = self.refinment_head(pred)
             
            losses_occupancy = self.refinment_head.loss(
                output_voxels=output['ssc_logit'],
                target_voxels=gt_occ,
            )
            losses.update(losses_occupancy)
            
            pred = output['ssc_logit']
            pred = torch.argmax(pred, dim=1)
